...
from adaptor.ocr import OCR

from adaptor.android.adb_shell import ADBShell
from adaptor.android.inputemu import get_model, get_ident, playback_gesture
...
class AndroidAssistant:
    ...
    def make_move(self, move):
        ...
        if self.last_board is not None:
            ...
            if sorted(self.last_board.flatten()) == sorted(board.flatten()):
                ...
                sleeptime = 0.5
            elif top <= 3 or list(self.last_board.flatten()).count(top) == list(
                board.flatten()
            ).count(top):
    ...
    def restart(self):
        ...
        time.sleep(2)
        playback_gesture(
            self.shell, self.ident, "pressbutton"
        )  # press Main Menu button
        time.sleep(2)
        playback_gesture(
            self.shell, self.ident, "pressbutton"
        )  # press Play Threes button
        time.sleep(2.0)
        self.last_board = None
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Control Threes! running on an Android phone"
    )
    parser.add_argument(
        "--no-resume",
        action="store_false",
        dest="resume",
        default=True,
        help="Don't resume from previous data",
    )
    parser.add_argument(
        "--from-start",
        action="store_true",
        default=False,
        help="Assume that the game starts from the initial state. May improve performance.",
    )
    parser.add_argument(
        "--repeat", action="store_true", default=False, help="Repeat games indefinitely"
    )
    parser.add_argument(
        "outdir",
        nargs="?",
        help="Output directory for screen captures (interpreted as a prefix if repeat is on)",
    )
    ...
    for i, suffix in enumerate(iterations):
        ...
        if args.outdir:
            ...
            try:
                ...
                pass
            run_assistant(
                assistant.gen_board_disk(outdir, args.resume),
                assistant.make_move,
                args.from_start,
            )
        else:
            run_assistant(
                assistant.gen_board_mem(), assistant.make_move, args.from_start
            )
