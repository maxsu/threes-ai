...
def guessWebImplementation(ctrl):
    ...
    if ctrl.execute("typeof(document.THREE)") != "undefined":
        ...
        raise ValueError(
            "Not yet able to support remote control of "
            + ctrl.execute('"" + window.location')
        )
...
def parse_args(argv):
    ...
    parser.add_argument(
        "-b",
        "--browser",
        help="Browser you're using. Only Firefox with the Remote Control extension, and Chrome with remote debugging, are supported right now.",
        default="firefox",
