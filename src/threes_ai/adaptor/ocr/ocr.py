...
class ExemplarMatcher:
    ...
    def guess_classify(self, imc):
        ...
        if len(possible) == 1:
            ...
            print(
                "Warning: multiple matches %s; guesser may not be accurate!" % possible
            )
        return None
    ...
    def classify(self, imc):
        ...
        if val is not None:
            ...
            imc.show()
            val = input(
                "\aUnrecognized %s! Recognize it and type in the value: " % self.tag
            )
...
class OCR:
    def __init__(self, model):
        if model not in CONFIGS:
            raise ValueError(
                "Configuration for model %s not found: please add it in devices.py"
                % model
            )
