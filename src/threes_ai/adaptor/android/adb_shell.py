...
class ShellCommandException(OSError):
    ...
    def __str__(self):
        if self.status > 128:
            return "Command '%s' was killed by signal %d" % (
                self.cmd,
                self.status - 128,
            )
        else:
    ...
    def __init__(self, opts=None):
        ...
        while time.time() - start < 5.0:
            ...
            if not s:
                ...
                if res is not None:
                    raise OSError(
                        "Failed to start '%s': process returned %d"
                        % (" ".join(cmd), res)
                    )
                time.sleep(0.01)
                continue
            ...
            if prompt.endswith(b"$ ") or prompt.endswith(b"# "):
                ...
                self.proc.stdin.write(b"\x1b[%d;%dR" % (10000000, 10000000))
        ...
        if m:
            ...
            self.prompt_re = re.compile(
                (
                    r"(?:(?P<status>\d+)\|)?(?P<user>%s|root)@%s:(?P<cwd>.*?) (?P<hash>[$#]) $"
                    % (self.user, self.host)
                ).encode()
            )
        else:
    ...
    def execute(self, cmd, text=False):
        ...
        if d.get("status"):
            raise ShellCommandException(
                self._encode_command(cmd), int(d["status"]), ret.decode()
            )
